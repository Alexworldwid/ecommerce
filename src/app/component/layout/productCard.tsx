...
const ProductCard = ({ product }: ProductCardProps) => {
  const { name, image, price, in_stock, id } = product;
    ...
    <Link key={id} href={`/products/${id}`} className="w-full min-w-64 px-2 py-4 rounded inline-flex flex-1 flex-col justify-start items-start gap-6 overflow-hidden">
      ...
      <div className='flex flex-col justify-start items-start gap-2 w-full'>
        ...
        <div className='flex items-center gap-2'>
          <p className="">
            {in_stock ? (
              <span className="px-4 py-0.5 rounded-[100px] outline outline-1 outline-offset-[-1px] outline-gray-200 inline-flex justify-start items-start gap-2 overflow-hidden text-green-600 text-xs font-medium font-inter capitalize leading-normal">In Stock</span>
            ) : (
